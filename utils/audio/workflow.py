...
def read_wav_len(wav_path):
    ...
    nchannels, sampwidth, framerate, nframes = params[:4]
    wave_read.close()
    print(wav_path,'=>',framerate,"==",nframes)
    return framerate * nframes

def batch_read_wav(file_dir):
    """
    从文件夹里面读取各个字母/数字对应的wav文件
    """

    pass

def feature_extraction(wav_data):
    """
    传统的机器学习特征提取方法
...
def preprocessing(wav_path):
    ...
    print("f=>", np.shape(f), f)
    print("t=>", np.shape(t), t)
    print("Zxx=>", np.shape(Zxx), Zxx)
...
def wav_files_stats(wavs_dir):
    import os
    for wav_path in os.listdir(wavs_dir):
        read_wav_len(os.path.join(wavs_dir, wav_path))

def audio_to_picture(wav_path):
    """
    将音频数据转成多通道图片
    ...
    def surf(x, y, Z):
        ...
        # border = np.ceil(300/(48000/nfft))
        border = 300
        fig = plt.figure()
        ax = fig.gca(projection='3d', proj_type='ortho',)
        ax.set_ylim3d(19000-border,19000+border)
        ax.axis('off')
        ax.grid(False)
        ...
        scalarMap = cm.ScalarMappable(norm=Normalize(vmin=C.min(), vmax=C.max()), cmap=plt.get_cmap("parula")) #cm.RdYlGn cm.jet, etc
        ...
        # xx, yy = np.meshgrid(range(np.shape(Z)[1]), range(np.shape(Z)[0]))
...
import matplotlib.pyplot as plt
def audio_to_picture2(wav_path):
    f, t, PP = preprocessing(wav_path)
    plt.figure()
    plt.set_cmap('parula')
    ...
    plt.savefig("./test6-6.jpg", dpi=96)
    plt.show()
...
if __name__ == "__main__":
    print("Acoudigit initialization")
    # batch_read_wav("D:\AcouDigits\journalExtension\AcouDigits_声音数据_解压\HSC")
    # audio_to_picture("D:\AcouDigits\journalExtension\AcouDigits_声音数据_解压\HSC\\0\\rec2018-12-21_22h06m25.692s.wav")
    # audio_to_picture2("D:\AcouDigits\journalExtension\AcouDigits_声音数据_解压\HSC\\0\\rec2018-12-21_22h06m25.692s.wav")
    ...
    from matplotlib import cm
    # LinearSegmentedColormap('parula', cm_data)
