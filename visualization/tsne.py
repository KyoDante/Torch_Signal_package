...
class tsne:
    ...
    def __init__(self, X:torch.Tensor, perp=30):
        super().__init__()
        self.X = X
        ...
        torch.manual_seed(1)
        self.Y = (torch.randn(self.N, 2)*1e-4).requires_grad_()
        self.perp = perp

    def cal_distance(self, data:torch.Tensor):
        '''
        计算欧氏距离
        ...
        '''
        assert data.dim() == 2, '应该为N*features'
    ...
    def cal_Q(self, data):
        ...
        # 对角线强制为零
        Q[torch.eye(self.N, self.N, dtype=torch.long) == 1] = 0
        Q = Q/Q.sum()
...
def example():
    ...
    X = torch.load('./visualization/data/x.pt').float()  # 0,1二值化的
    X = torch.from_numpy(PCA(n_components=30).fit_transform(X.numpy())).float()
    C = torch.load('./visualization/data/labels.pt').float()
    X = X[0:600]
    C = C[0:600]
